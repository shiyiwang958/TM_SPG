...
def reorder_by_level_halves(data: Dataset):
    """
    Takes the mapped dataset returned by your function and returns:
      - one dataset where items with level in {0,1,2} come first (stable),
        followed by items with level in {3,4,5} (stable),
      - split_idx: the boundary index (len(first_half))
    Original relative order is preserved within each group.
    """
    # Pull the column once (fast) to build stable index lists
    levels = data["level"]

    first_idx = [i for i, lvl in enumerate(levels) if lvl in (0, 1, 2)]
    second_idx = [i for i, lvl in enumerate(levels) if lvl in (3, 4, 5)]

    split_idx = len(first_idx)
    reordered = data.select(first_idx + second_idx)

    return reordered, split_idx

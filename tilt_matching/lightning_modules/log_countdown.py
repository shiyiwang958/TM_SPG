    ...
    def logging_student(self, model, num_buffer_updates, num_completions_per_prompt):
        ...
        for i in range(num_val_prompts):
            ...
            for comp_idx in range(num_completions_per_prompt):
                ...
                # Extract the equation/answer from completion
                equation = None
                equation = self.extract_solution(completion_text)
        ...
        # and the number of correct equations that uses division /
        # and the number of correct equations that uses non of these
        num_paren = 0
        num_mult = 0
        num_div = 0
        num_add_subtract = 0
        ...
        for i in range(num_val_prompts):
            for j in range(num_completions_per_prompt):
                ...
                # Only count if the equation has max reward (is correct)
                reward_val = rewards_per_func[global_idx, 0].item() if num_funcs > 0 else 0.0
                if reward_val >= self.hparams.max_rwd - 1e-5:
                    completion_text = completions_text[global_idx]
                    equation = self.extract_solution(completion_text)
        ...
        self.log("frac_num_mult", frac_num_mult, on_step=True, on_epoch=False, sync_dist=True)
        self.log("frac_num_div", frac_num_div, on_step=True, on_epoch=False, sync_dist=True)
        self.log("frac_num_add_subtract", frac_num_add_subtract, on_step=True, on_epoch=False, sync_dist=True)

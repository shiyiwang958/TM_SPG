...
def parse_gsm_answers(json_path=None, json_data=None):
    ...
    for item in data.get("generations", []):
                    ...
                    try:
                        ...
                        if numbers:
                            try:
                                ...
                                pass
        if parsed_answer is None:
            answer = raw_generation.split("<answer>")[-1]
        ...
        if "nfes" in item.keys():
            processed_items[-1]["nfes"] = item["nfes"]
...
def aggregate_results(directory="."):
    ...
    for setup, results in sorted(setups.items()):
        ...
        if "nfes" in results["questions"][0].keys():
            results["avg_nfes"] = sum(q["nfes"] for q in results["questions"]) / len(results["questions"])
        else:
            results["avg_nfes"] = 128
    # Header
    header_format = "{:<40} {:>12} {:>25} {:>25}"
    print(header_format.format("Setup (task_model_seqlen_diffusteps)", "Accuracy", "Avg Effective Tokens", "Avg NFE"))
    print("-" * 80)

    # Data rows
    row_format = "{:<40} {:>11.2f}% {:>25.2f} {:>25.0f}"
    for setup, results in sorted(setups.items()):
        print(row_format.format(setup, results["accuracy"], results["avg_effective_tokens"], results["avg_nfes"]))

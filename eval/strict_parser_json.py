...
def extract_xml_answer(text: str) -> str:
    # Match reward_func.py exactly
    answer = text.split("<answer>")[-1]
    answer = answer.split("</answer>")[0]
    return answer.strip()


def gsm_ground_truth_to_str(ground_truth) -> str:
    """
    Make GT string match what reward_func.py compares against, GIVEN that
    your generations put \\boxed{...} inside <answer>...</answer>.
    """
    if ground_truth is None:
        return ""

    # If GT already has \boxed{...}, keep it.
    ...
    # JSON stores numbers as floats in your samples.json
    if isinstance(ground_truth, float) and ground_truth.is_integer():
        val = str(int(ground_truth))
    else:
        val = str(ground_truth).strip()

    return f"\\boxed{{{val}}}"



def parse_gsm_answers(json_path=None, json_data=None, strict: bool = True):
    if json_path:
    ...
    for item in data.get("generations", []):
        ...
        if strict:
            extracted_answer = extract_xml_answer(raw_generation)
            gt_str = gsm_ground_truth_to_str(ground_truth)

            is_correct = extracted_answer == gt_str
            if is_correct:
                total_correct += 1
